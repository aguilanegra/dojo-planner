...
import type { ClassTag } from './classTagsData';
...
export function ClassTagsManagement({ open, onOpenChange }: ClassTagsManagementProps) {
  ...
  const filteredTags = useMemo(() => {
    if (!searchQuery.trim()) {
      return mockClassTags;
    }
    const query = searchQuery.toLowerCase();
    return mockClassTags.filter(
      tag =>
        tag.name.toLowerCase().includes(query)
        || tag.classNames.some(className => className.toLowerCase().includes(query)),
    );
  }, [searchQuery]);
