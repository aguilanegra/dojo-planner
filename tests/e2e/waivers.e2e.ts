...
test.describe('Waivers Management', () => {
  ...
  test('should create a new active waiver template', async ({ page }) => {
    ...
    // Dialog is tall — button may be outside viewport; use dispatchEvent to bypass viewport check
    await page.getByRole('button', { name: /create waiver/i }).dispatchEvent('click');
  ...
  test('should navigate to waiver detail and see delete button', async ({ page }) => {
    ...
    // Dialog is tall — button may be outside viewport; use dispatchEvent
    await page.getByRole('button', { name: /create waiver/i }).dispatchEvent('click');
    await page.waitForLoadState('domcontentloaded');
